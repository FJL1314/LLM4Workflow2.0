...
CREATE TABLE workflow_rubic (
    id SERIAL PRIMARY KEY,
    uid INT REFERENCES userInfo (uid),
    workflow_id INT REFERENCES workflow (id) UNIQUE,
    task JSONB NOT NULL,
    sim_results JSONB,
    report JSONB,
    final_rubric JSONB,
...
ALTER TABLE workflow_rubic
ADD CONSTRAINT unique_workflow_id UNIQUE (workflow_id);
